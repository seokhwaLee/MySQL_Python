...
def connect_curs():
    connect = pymysql.connect(host = 'localhost', user= 'teamproject', password = 'project', db = 'shoppingmall', charset = 'utf8')
    curs = connect.cursor(pymysql.cursors.DictCursor)
    return connect, curs

def close_conn():
    pymysql.connect(host = 'localhost', user= 'teamproject', password = 'project', db = 'shoppingmall', charset = 'utf8').close()
...
def revise_data():
    ...
    if what == '고객':
        ...
        connect, curs = connect_curs()
        id=input('아이디를 입력해주세요 >> ')
        pww=input('변경할 비밀번호를 입력해주세요 >> ')
        sql='update customer set passwd = %s where cus_id = %s'
        curs.execute(sql, (pww, id))
        connect.commit()
        close_conn()
